  ...
  return (
    <section className="relative bg-secondary overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        ...
        {/* Search card */}
        <div className="bg-white rounded-xl shadow-2xl overflow-hidden border-4 border-accent">
          {/* Tabs */}
          ...
          <form onSubmit={handleSearch} className="flex gap-2 p-3">
            ...
            <Button type="submit" className="shrink-0 px-7 bg-accent hover:bg-amber-500 text-white font-semibold rounded-lg">
              Search
            </Button>
          </form>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4">
          ...
          {QUICK.map((place) => (
            <button
              ...
              className="text-xs text-white/80 hover:text-white bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/40 rounded-full px-3 py-1 transition-colors"
            >
              {place}
            </button>
          ))}
